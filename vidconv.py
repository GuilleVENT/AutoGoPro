...
class VideoConverter:
    ...
    def __init__(
        ...
        ----------
            ...
            - 'crop': Boolean indicating if video should be cropped for portrait mode on phones. (bool)
            - 'remove': Boolean indicating if the input file should be removed after conversion. (bool)
            ...
            - 'rm_audio': Whether to remove audio track of the video. (bool)
            - 'keep_gps': Whether to preserve GPS metadata in the output video. (bool)
            - 'extract_gpx': Whether to extract GPS data into a separate GPX file. (bool)
            - 'overwrite': Whether or not to overwrite if there's a file with the same name already in the target folder. (bool) 
            - 'bright': Whether to apply brightness and saturation adjustments.
        """
        
        # Store flags in class
    ...
    def validate_options(self):
        ...
        # 1. Resolution
        valid_resolutions = ['unchanged', '360', '480', '720', '1080', '2160']
        levels = ['low', 'mid', 'high']
        for level in levels:
        ...
        # If resolution is not 'unchanged', ensure it's in BITRATES
        if self.resolution != 'unchanged':
            for level in levels:
        ...
        if not isinstance(self.overwrite, bool):
            raise ValueError("Option 'overwrite' must be True or False.")

        # 13. bright (boolean)
        if not isinstance(self.bright, bool):
            raise ValueError("Option 'bright' must be True or False.")
    ...
    def should_process_video(self, input_file):
        ...
        try:
            ...
            # Handle resolution and cropping
            if self.crop:
                # Calculate target width based on 9:16 aspect ratio and original height
                target_width = int(current_height * 9 / 16)
                # Ensure target_width is even
                target_width = target_width - (target_width % 2)
                if self.resolution != 'unchanged':
                    target_height = int(self.resolution)
                    if current_height != target_height:
                        process_flags['resolution'] = True  # Scaling is needed
                else:
                    # When resolution is unchanged, set target_height to current
                    target_height = current_height
                    process_flags['resolution'] = False  # No scaling

                # If cropping is enabled, check if target_width matches the current width
                if target_width == video_stream['width']:
                    process_flags['resolution'] = False  # No cropping needed
                else:
                    # Cropping will be applied
                    process_flags['resolution'] = process_flags['resolution']

            else:
                # If not cropping, handle resolution normally
                if self.resolution == 'unchanged':
                    target_height = current_height
                    process_flags['resolution'] = False  # No scaling
                else:
    ...
    def prepare_strings_outfiles(self, in_file, flags):
        ...
        # Handle 'unchanged' resolution with cropping
        if self.resolution == 'unchanged' and self.crop:
            resolution_str = "portrait"
        else:
            resolution_str = f"{self.resolution}p" if flags['resolution'] and self.resolution != 'unchanged' else ""
        ...
        if flags['bitrate']:
            if self.bitrate in BITRATES:
                # Determine resolution key based on whether resolution is unchanged
                current_info = self.get_video_info(in_file)
                video_stream = [stream for stream in current_info['streams'] if stream['codec_type'] == 'video'][0]
                current_resolution = str(video_stream['height'])
                res_key = self.resolution if self.resolution != 'unchanged' else current_resolution
                bitrate_in_mbps = int(BITRATES[self.bitrate].get(res_key, '0k').replace('k', '000')) / 1000000
        ...
        # GPS flags
        gps_str = "keepGPS" if self.keep_gps else ""
        extract_gpx_str = "extractGPX" if self.extract_gpx else ""

        # brightness
        bright_str = "bright" if self.bright else ""

        # Combine relevant components for the filename
        components = [codec_str, quality_str, resolution_str, bitrate_str, rm_audio_str, crop_str, gps_str, extract_gpx_str, bright_str]
    ...
    def run_ffmpeg_conversion(self, input_file, output_file, flags):
        ...
        if self.bitrate != 'unchanged' and flags['bitrate']:
            if self.bitrate in BITRATES:
                # Determine resolution key based on whether resolution is unchanged
                current_info = self.get_video_info(input_file)
                video_stream = [stream for stream in current_info['streams'] if stream['codec_type'] == 'video'][0]
                current_resolution = str(video_stream['height'])
                res_key = self.resolution if self.resolution != 'unchanged' else current_resolution
                bitrate = BITRATES[self.bitrate].get(res_key, None)
                if bitrate:
                    cmd.extend(["-b:v", bitrate])
                else:
                    logging.warning(f"Undefined bitrate for resolution '{res_key}' and bitrate level '{self.bitrate}'.")
            else:
                # Assume it's a numeric value like '4000k' or '4M'
        ...
        if self.crop:
            ...
            # Calculate target width based on 9:16 aspect ratio and original height
            # Using the original height to determine the width
            # Ensuring the width is even
            current_info = self.get_video_info(input_file)
            video_stream = [stream for stream in current_info['streams'] if stream['codec_type'] == 'video'][0]
            ...
            # Calculate x offset to center the crop
            x_offset = int((current_width - target_width) / 2)
            x_offset = x_offset - (x_offset % 2)  # Ensure even offset

            crop_filter = f"crop={target_width}:{current_height}:{x_offset}:0"
            vf_options.append(crop_filter)
        
        if flags['resolution'] and self.resolution != 'unchanged':
        ...
        elif not self.crop and self.resolution != 'unchanged':
            # If not cropping but resolution is changed
            vf_options.append(f"scale=-2:{self.resolution}")

        if self.bright:
            bright_filter = "eq=gamma=1.5:saturation=1.0"
            vf_options.append(bright_filter)
        ...
        # Handle forcefully overwriting
        if self.overwrite: 
            cmd.append("-y") # overwrite if it exists here
        else: 
            cmd.append("-n") # don't overwrite -> default : FAILS IF FAIL EXISTS

        cmd.append(output_file) # specify output file 

        logging.info(f"Running ffmpeg on {input_file} with:")
        logging.info(f" >> {' '.join(cmd)}")
        
        # Execution of FFMPEG with error handling
        try:
    ...
    def run_file_conversion(self, in_file):
        ...
        if in_file.split('.')[-1].lower() in VALID_VIDEO_EXTENSIONS:
            ...
            if flags['valid_filetype'] and any([
                    flags['codec'], flags['resolution'], flags['bitrate'], flags['keep_gps'], flags['extract_gpx'], flags['bright']]):                
...
def main():
    ...
    parser.add_argument("--resolution", choices=["unchanged", "360", "480", "720", "1080", "2160"], default="1080", help="Target video resolution. Don't give '720p' with `p'. Default '1080'. Use 'unchanged' to keep original resolution.")    
    ...
    parser.add_argument("--overwrite", action="store_true", help="Whether to force an overwrite if the file already exists.")
    parser.add_argument("--bright", action="store_true", help="Increase brightness and adjust saturation of the video.")    
    ...
    ## if INPUT FOLDER:
        ...
        if args.remove:
            confirm = input(f"After conversion, you will be removing all input files in the directory '{args.input_path}' and its subdirectories. Are you sure? (then type yes or y): ")
